    ...
    public static void add(Room[] rooms) {
        try{
            ...
            if(rooms[n].getGuestsInRoom() == 0){
                ...
                if (isFull(rooms)){
                    System.out.println("Room is already Occupied! You will be Added to Waiting List!\n");
                    Room room = new Room();
                    addCustomer(room);
                    waitingList.enQueue(room);
                } else {
                    System.out.println("Room is already occupied! Try another Room!");
                }
            }
            ...
            if (rooms[n].getGuestsInRoom() == 0) {
                ...
                if(!waitingList.isEmpty()){
                    ...
                    setRoomData(rooms[n],
                            ...
                            waitingCustomer.getPayingGuest().getCreditCardNumber());

                    System.out.println("A Customer from the Waiting List is Added to this Room!");
                }
            }
        } catch (Exception exception){
            /* If the user enters a invalid value for roomNum
